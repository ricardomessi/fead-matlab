...
for k_init=1:np
    c=PC{k_init};
    uilabel(lp,'Text',pnames{k_init},'Position',[5 y 38 22],...
        'FontColor',c,'FontWeight','bold','BackgroundColor',PANEL);
    S.ef_x{k_init}=uieditfield(lp,'numeric','Value',S.pulleys(k_init).x,...
        'Limits',[-600 600],'Position',[46 y 82 22],...
        'BackgroundColor',[0.07 0.10 0.18],'FontColor',TC,...
        'ValueChangedFcn',make_pedit_callback(@cb_pedit,k_init,'x'));
    S.ef_y{k_init}=uieditfield(lp,'numeric','Value',S.pulleys(k_init).y,...
        'Limits',[-100 700],'Position',[132 y 82 22],...
        'BackgroundColor',[0.07 0.10 0.18],'FontColor',TC,...
        'ValueChangedFcn',make_pedit_callback(@cb_pedit,k_init,'y'));
    S.ef_r{k_init}=uieditfield(lp,'numeric','Value',S.pulleys(k_init).r,...
        'Limits',[10 150],'Position',[218 y 82 22],...
        'BackgroundColor',[0.07 0.10 0.18],'FontColor',TC,...
        'ValueChangedFcn',make_pedit_callback(@cb_pedit,k_init,'r'));
    y=y-23;
end
y=y-6;
...
for k_init=1:np
    S.badge{k_init}=uilabel(cp,'Position',[(k_init-1)*136+5 5 131 36],...
        'Text',sprintf('%s\n—',pnames{k_init}),...
        'FontSize',9,'FontWeight','bold','FontColor',PC{k_init},...
...
function cb = make_pedit_callback(cb_fun, k, coord)
    cb = @(s,~)cb_fun(k, coord, s.Value);
end
